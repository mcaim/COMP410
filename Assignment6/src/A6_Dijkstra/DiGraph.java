...
public class DiGraph implements DiGraph_Interface {
	...
	public ShortestPathInfo[] shortestPath(String label) {
		...
		Vertex src_vertex = (Vertex) vertices.get(label);
		if (src_vertex == null) {
			return null;
		} else {
			src_vertex.setDist(0);
			...
			while (true) {
				...
				for (Edge e : v.outedges.values()) {
					double dk = v.distance + (double) e.weight;
					if (dk < e.destination.distance) {
						e.destination.setParent(v);
						heap.decreaseKey(dk, e.destination);
					}
				}
				
				long weight = (long) v.distance;
				if (weight == Long.MAX_VALUE) {
					weight = -1;
				}

				ShortestPathInfo curr = new ShortestPathInfo(v.label, weight);
				path[count] = curr;
				++count;
			}
		}
